...
一个 Windows 系统托盘应用程序，用于智能控制输入法状态。解决 Windows 中文输入法在某些场景下频繁自动切换的苦恼（比如切换到终端，智障Windows会切换为中文输入法），支持强制锁定中文或英文模式，并提供便捷的快捷键切换功能。
...
- 🎯 **智能事件检测**：监听窗口切换事件，精准控制输入法状态
- 🔄 **全局状态管理**：强制统一所有窗口的输入法状态，避免各自为政
...
> 🔥 **核心功能**：程序采用全局输入法状态管理机制，确保无论切换到哪个窗口，输入法状态始终保持一致！

### 工作原理

- **窗口切换监听**：当用户切换到不同程序窗口时触发状态检查
- **全局状态强制**：检测到不符合配置的输入法状态时，立即强制统一
- **多层保障机制**：通过多种Windows API确保状态同步到所有窗口

### 应用场景

- ✅ 解决微软拼音输入法在不同应用间自动切换中英文的问题
- ✅ 确保切换到终端、IDE等应用时输入法状态保持一致
- ✅ 避免游戏、聊天软件等场景下的输入法意外切换
- ✅ 任何窗口的输入法状态变化都会被检测并统一为预设状态

## 快捷键说明(可自行更改)

- **Alt+M**：切换软件总开关（启用/禁用输入法控制）
- **CTRL+SPACE**：在中文模式和英文模式之间切换

> 💡 **提示**：可以通过编辑配置文件自定义快捷键组合
>
> ⚠️ **不建议**把中英文模式切换绑定为 `CAPSLOCK`：单击即可在"强制中文/强制英文"之间整体翻转，
> 极易误触导致输入法突然变成英文（大写），老版本配置文件中若仍是 `CAPSLOCK`，建议改为 `CTRL+SPACE` 等组合键。
...
- **操作系统**：Windows 10/11
- **输入法**：微软拼音输入法（推荐）或其他支持系统输入法API的输入法
- **权限**：无需管理员权限
...
1. 确认当前使用的是微软拼音输入法或其他兼容输入法
2. 检查软件功能是否已启用（托盘图标应为彩色）
3. 尝试手动切换一次模式
4. 查看日志文件确认输入法状态检测是否正常工作
